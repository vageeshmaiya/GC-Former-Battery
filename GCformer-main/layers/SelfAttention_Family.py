...
class FullAttention(nn.Module):
    def __init__(self,pred_len, mask_flag=True, factor=5, scale=None, attention_dropout=0.3, output_attention=False):
        super(FullAttention, self).__init__()
        self.scale = scale
        self.mask_flag = mask_flag
        self.output_attention = output_attention
        self.dropout = nn.Dropout(attention_dropout)
        self.qk_mask = nn.Parameter(torch.ones(pred_len, pred_len))
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        #print("Q_K_sample.max(-1)[0]", Q_K_sample.max(-1)[0])
        #print("Q_K_sample.sum(-1)", Q_K_sample.sum(-1))
...
activation_dict = {"ReLU": torch.nn.ReLU(), "Softplus": torch.nn.Softplus(), "Swish": swish,
                   "GELU": torch.nn.GELU(), "Softmax": torch.nn.Softmax}
...
class LogSparseAttention(nn.Module):
    def __init__(self, n_head, n_embd, win_len, scale, q_len, sub_len, sparse=None, attn_pdrop=0.1, resid_pdrop=0.1):
        ...
        self.scale = scale
        self.q_len = q_len
        self.query_key = nn.Conv1d(n_embd, n_embd * n_head * 2, 1)
    ...
    def forward(self, qk, q, v):

        #pdb.set_trace()
        value = self.value(v)
        qk_x = nn.functional.pad(qk.permute(0, 2, 1), pad=(self.q_len - 1, 0))
